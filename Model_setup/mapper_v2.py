...
RTS = [100] #Number of nodes to retain (can be a single number or series of numbers to create different topologies automatically)
...
distance_threshold2_load = 80 #Distance threshold in km, restricts algorithm to select any two nodes that are closer than this number (this is used after we selected a single node in each BA)
    ...
    while l_N > 0:
        ...
        if my_state in coast_states:
            ...
            for d in inland_state_nodes:
                ...
                if dist < distance_threshold2_load:
